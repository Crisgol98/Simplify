...
function addTask() {
    ...
    let hoursOptions = '';
    for (let i = 0; i < 24; i++) {
        hoursOptions += `<option value="${i}">${String(i).padStart(2, '0')}</option>`;
    }

    let minutesOptions = '';
    for (let i = 0; i < 60; i += 5) {
        minutesOptions += `<option value="${i}">${String(i).padStart(2, '0')}</option>`;
    }
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.success) {
                ...
                let hoursOptions = '';
                for (let i = 0; i < 24; i++) {
                    const selected = i === workedHours ? 'selected' : '';
                    hoursOptions += `<option value="${i}" ${selected}>${String(i).padStart(2, '0')}</option>`;
                }

                let minutesOptions = '';
                for (let i = 0; i < 60; i += 5) {
                    const selected = i === workedMinutes ? 'selected' : '';
                    minutesOptions += `<option value="${i}" ${selected}>${String(i).padStart(2, '0')}</option>`;
                }
                        ...
                        <form id="updateStateForm">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control" id="taskState" name="state">
                                    ...
                                    <option value="Finalizado" ${currentState === 'Completado' ? 'selected' : ''}>Completada</option>
                                </select>
                            </div>
                            <div class="mb-3">
                                ...
                                <div class="d-flex justify-content-center">
                                    <div class="me-2 text-center">
                                        <label for="workedHours" class="form-label d-block">Horas</label>
                                        <select class="form-control form-control-sm" id="workedHours" name="workedHours" required>
                                            ${hoursOptions}
                                        </select>
                                    </div>
                                    <div class="text-center">
                                        <label for="workedMinutes" class="form-label d-block">Minutos</label>
                                        <select class="form-control form-control-sm" id="workedMinutes" name="workedMinutes" required>
                                            ${minutesOptions}
                                        </select>
                                    </div>
                                </div>
                ...
                Swal.fire({
                    ...
                    cancelButtonColor: '#d33',
                    preConfirm: () => {
                        const hours = parseInt($('#workedHours').val());
                        const minutes = parseInt($('#workedMinutes').val());
                        ...
                        <div class="task-details__info mb-4">
                            ...
                            <p><strong>Estado:</strong> 
                                <span class="badge ${response.task.isCompleted ? 'bg-success' : 'bg-warning'}">
                                    ${response.task.isCompleted ? 'Completada' : 'Pendiente'}
                                </span>
                            </p>
