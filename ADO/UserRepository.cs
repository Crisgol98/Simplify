                        ...
                        try
                        {
                            ...
                            using (var cmd = new SqlCommand(preferencesQuery, con, transaction))
                            {
                                ...
                                using (var reader = await cmd.ExecuteReaderAsync())
                                {
                                    ...
                                    {
                                        preferences.StartTime = reader.IsDBNull(reader.GetOrdinal("StartTime")) ? TimeSpan.Zero : reader.GetTimeSpan(reader.GetOrdinal("StartTime"));
                                        preferences.EndTime = reader.IsDBNull(reader.GetOrdinal("EndTime")) ? TimeSpan.Zero : reader.GetTimeSpan(reader.GetOrdinal("EndTime"));
                                        preferences.BreakLength = reader.IsDBNull(reader.GetOrdinal("BreakLength")) ? TimeSpan.Zero : reader.GetTimeSpan(reader.GetOrdinal("BreakLength"));
                                        preferences.BreakFrequency = reader.IsDBNull(reader.GetOrdinal("BreakFrequency")) ? TimeSpan.Zero : reader.GetTimeSpan(reader.GetOrdinal("BreakFrequency"));
                                    }
                                }
                            }
